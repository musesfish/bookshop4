...
router.get("/del/:bid",(req,res)=>{
    let bid = req.params.bid;
    pool.query("DELETE FROM bs_book WHERE bid=?",bid, (err, result)=>{
      if(err)throw err;
...
router.get('/list/:pageNum', (req, res)=>{
  ...
  pool.query("SELECT COUNT(*) AS c FROM bs_book", (err, result)=>{
    if(err)throw err;
    pager.totalRecord = result[0]['c'];//总记录数
    pager.pageCount = Math.ceil(pager.totalRecord/pager.pageSize);//总页数
    pool.query("SELECT * FROM bs_book ORDER BY bid LIMIT ?,?",[(pager.pageNum-1)*pager.pageSize,  pager.pageSize], (err, result)=>{
      if(err)throw err;
